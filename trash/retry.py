...
c3.metric("Peak Congestion", round(df_cong["congestion_index"].max(), 1))
c4.metric("📊Market Leader", df_agency.iloc[0]["agency_name"], f"{df_agency.iloc[0]['percentage']}%")
c5.metric(
        "Longest Route",
        longest["route_id"],
        f"{longest['duration']:.1f} min"
    )

c6.metric(
        "Shortest Route",
        shortest["route_id"],
        f"{shortest['duration']:.1f} min"
    )
...
if selected_routes:
    ...
    route_id = routes_df[
        routes_df["agency_id"] == selected_agencies[0]
    ].iloc[0]["route_id"]
...
with col_map2:
    ...
    with st.container(border=True):
        ...
        # Major Starting Stops
        st.markdown("### 🚌 Major Starting Stops")
...
with right_container:
    ...
    with st.container(border=True):
        ...
        with col1:
            ...
            block = (
                df_cong
                .groupby("time_block", as_index=False)["congestion_index"]
                .mean()
            )
