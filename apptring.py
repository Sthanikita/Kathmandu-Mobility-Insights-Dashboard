...
col_filter, col_map1, col_map2 = st.columns([1, 3, 1])
...
with col_filter:
    ...
    for _, agency_row in agencies.iterrows():
        ...
        st.markdown("""<div style="padding:10px;background:#fafafa;border-radius:10px;margin-bottom:10px;">""", unsafe_allow_html=True)

        agency_checked = st.checkbox(f"🏢 {agency_name}", key=f"agency_{agency_id}")
        ...
        if agency_checked:
            ...
            for _, route_row in agency_routes.iterrows():
                ...
                if route_checked:
                    ...
                    route_selected_flag = True

            if route_selected_flag:
