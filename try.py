...
def build_transit_map(selected_routes, route_color_map, route_name_map,
    ...
    and an aspect-locked axis pair that stays centered on zoom/pan/rerun."""
    fig = go.Figure()
    ...
    fig.update_layout(
        ...
        legend=dict(
            ...
            x=1.0,
            y=1.0,
            xanchor="left",   # anchored to the LEFT of the legend box,
            yanchor="top",    # placed at x=1.0 -- i.e. just outside the
                              # plot's right edge in paper space, not
                              # floating over the plot area itself. This is
                              # what was covering "Basundhara Bus Stop":
                              # the old legend had no xref/yref/x/y, so
                              # Plotly auto-placed it inside the plot area,
                              # and it could land on top of whichever
                              # route happened to be drawn up there.
        ),
        xaxis=dict(
            visible=False,
            scaleanchor="y",
            scaleratio=1,
            constrain="domain",       # keeps the 1:1-locked axis centered in
                                      # its domain instead of ballooning the
                                      # range (and hiding traces) on zoom
            constraintoward="center", # WITHOUT this, Plotly shrinks the axis
                                      # toward the left edge of its domain by
                                      # default -- that's the "map is shoved
                                      # top-left with blank space" you saw
        ),
        yaxis=dict(
            visible=False,
            constrain="domain",
            constraintoward="middle", # same fix on the vertical axis
        ),
        autosize=True,
        height=850,
        margin=dict(t=60, b=20, l=20, r=170),  # r=170 reserves space so the
                                               # paper-space legend at x=1.0
                                               # sits in dedicated margin,
                                               # never overlapping the plot
        hovermode="closest",
        dragmode="pan",
        uirevision="transit-map",  # preserves zoom/pan state across
                                   # Streamlit reruns (checkbox toggles,
                                   # pie-chart clicks, etc.)
    )
...
# =========================================================
# REAL LOOM PIPELINE (ad-freiburg/loom, via Docker)
# https://github.com/ad-freiburg/loom
# Pipeline: gtfs2graph -> topo -> loom -> [octi] -> transitmap -> SVG
# =========================================================
LOOM_DOCKER_IMAGE = "loom"

# named color -> hex, so the SVG loom renders uses the SAME colors as the
# rest of this dashboard (loom reads color from GTFS routes.txt route_color)
LOOM_COLOR_HEX = {
    "blue":   "1f77ff",
    "red":    "e6194b",
    "green":  "3cb44b",
...
def _write_gtfs_subset_zip(selected_routes, route_color_map, out_zip_path):
    """Builds a minimal, valid GTFS zip containing only the selected routes
    (and their trips/stop_times/stops/agency), with route_color set to match
    this dashboard's color scheme, so gtfs2graph has something to consume."""
    engine = get_engine()

    route_list = ",".join([f"'{r}'" for r in selected_routes])
    routes = pd.read_sql(f"SELECT * FROM routes WHERE route_id IN ({route_list})", engine)

    if routes.empty:
        raise ValueError("No routes found for the current selection.")

    # tag each route with a hex color matching our existing color legend
    routes["route_color"] = routes["route_id"].map(
        lambda rid: LOOM_COLOR_HEX.get(route_color_map.get(rid, "blue"), "1f77ff")
    )
    routes["route_text_color"] = "ffffff"
    if "route_type" not in routes.columns:
        routes["route_type"] = 3  # 3 = bus, per GTFS spec

    trip_ids_df = pd.read_sql(f"SELECT * FROM trips WHERE route_id IN ({route_list})", engine)
    trip_list = ",".join([f"'{t}'" for t in trip_ids_df["trip_id"]]) if not trip_ids_df.empty else "'__none__'"

    stop_times = pd.read_sql(f"SELECT * FROM stop_times WHERE trip_id IN ({trip_list})", engine)
    stop_ids = stop_times["stop_id"].unique().tolist() if not stop_times.empty else []
    stop_list = ",".join([f"'{s}'" for s in stop_ids]) if stop_ids else "'__none__'"

    stops = pd.read_sql(f"SELECT * FROM stops WHERE stop_id IN ({stop_list})", engine)

    agency_ids = routes["agency_id"].dropna().unique().tolist()
    agency_list = ",".join([f"'{a}'" for a in agency_ids]) if agency_ids else "'__none__'"
    agency = pd.read_sql(f"SELECT * FROM agency WHERE agency_id IN ({agency_list})", engine)
    ...
    tmp_dir = tempfile.mkdtemp(prefix="gtfs_subset_")
    try:
        routes.to_csv(os.path.join(tmp_dir, "routes.txt"), index=False)
        trip_ids_df.to_csv(os.path.join(tmp_dir, "trips.txt"), index=False)
        stop_times.to_csv(os.path.join(tmp_dir, "stop_times.txt"), index=False)
        stops.to_csv(os.path.join(tmp_dir, "stops.txt"), index=False)
        agency.to_csv(os.path.join(tmp_dir, "agency.txt"), index=False)
        calendar.to_csv(os.path.join(tmp_dir, "calendar.txt"), index=False)

        with zipfile.ZipFile(out_zip_path, "w") as zf:
            for fname in ["routes.txt", "trips.txt", "stop_times.txt",
                          "stops.txt", "agency.txt", "calendar.txt"]:
...
def generate_loom_svg(selected_routes_tuple, route_color_map, schematic: bool):
    """Runs the real ad-freiburg/loom pipeline in Docker on a GTFS subset
    built from the selected routes and returns (svg_text, error_text).
    schematic=True inserts the `octi` step for an octilinear schematic map;
    schematic=False renders the geographically accurate version."""
    selected_routes = list(selected_routes_tuple)
    work_dir = tempfile.mkdtemp(prefix="loom_")
    gtfs_zip = os.path.join(work_dir, "subset.zip")

    try:
        _write_gtfs_subset_zip(selected_routes, route_color_map, gtfs_zip)

        # 1) gtfs2graph: GTFS zip -> raw line graph (GeoJSON)
        p1 = subprocess.run(
            ["docker", "run", "--rm", "-v", f"{work_dir}:/data", "-i",
             LOOM_DOCKER_IMAGE, "gtfs2graph", "-m", "bus", "/data/subset.zip"],
            capture_output=True, check=True, timeout=120
        )

        # 2) topo: overlap-free line graph
        p2 = subprocess.run(
            ["docker", "run", "--rm", "-i", LOOM_DOCKER_IMAGE, "topo"],
            input=p1.stdout, capture_output=True, check=True, timeout=120
        )

        # 3) loom: optimal line ordering
        p3 = subprocess.run(
            ["docker", "run", "--rm", "-i", LOOM_DOCKER_IMAGE, "loom"],
            input=p2.stdout, capture_output=True, check=True, timeout=120
        )

        render_input = p3.stdout

        # 4) optional octi: schematic octilinear layout
        if schematic:
            p4 = subprocess.run(
                ["docker", "run", "--rm", "-i", LOOM_DOCKER_IMAGE, "octi"],
                input=render_input, capture_output=True, check=True, timeout=120
            )
            render_input = p4.stdout

        # 5) transitmap: render SVG with stop labels (-l)
        p5 = subprocess.run(
            ["docker", "run", "--rm", "-i", LOOM_DOCKER_IMAGE, "transitmap", "-l"],
            input=render_input, capture_output=True, check=True, timeout=120
        )

        return p5.stdout.decode("utf-8", errors="replace"), None

    except subprocess.CalledProcessError as e:
        err = e.stderr.decode("utf-8", errors="replace") if e.stderr else str(e)
        return None, err
    except FileNotFoundError:
        return None, "Docker command not found. Make sure Docker Desktop is installed and running."
    except subprocess.TimeoutExpired:
        return None, "The LOOM pipeline timed out."
    finally:
        shutil.rmtree(work_dir, ignore_errors=True)


def wrap_svg_centered(svg_text, title_text, route_name_map, route_color_map, selected_routes):
    """Wraps the raw LOOM SVG in HTML: centered title, centered map, and a
    legend built from this dashboard's own route names/colors underneath."""
    legend_items = ""
    for route_id in selected_routes:
        color_name = route_color_map.get(route_id, "blue")
        hex_color = LOOM_COLOR_HEX.get(color_name, "1f77ff")
        label = route_name_map.get(route_id, route_id)
        legend_items += f"""
            <div style="display:flex; align-items:center; gap:8px; margin:4px 14px;">
                <span style="width:16px; height:16px; border-radius:3px;
                             background:#{hex_color}; display:inline-block;
                             border:1px solid #ccc;"></span>
                <span style="font-family:Inter, sans-serif; font-size:13px; color:#111111;">{label}</span>
            </div>
        """

    return f"""
    <div style="width:100%; background:#ffffff; padding:24px 12px;
                display:flex; flex-direction:column; align-items:center;
                justify-content:center; border-radius:12px;">
        <h2 style="font-family:Inter, sans-serif; color:#111111; margin:0 0 6px 0;
                   text-align:center;">{title_text}</h2>
        <div style="display:flex; flex-wrap:wrap; justify-content:center;
                    margin-bottom:16px;">
            {legend_items}
        </div>
        <div style="max-width:100%; overflow:auto; display:flex; justify-content:center;">
            {svg_text}
        </div>
    </div>
    """
        ...
        with st.expander("Route Map", expanded=True):
            ...
            if not selected_routes:
                ...
                if view_mode == "🗺️ Live Map":
                    ...
                    # constrain="domain" + constraintoward="center"/"middle"
                    # in build_transit_map() now do the centering, so this
                    # can just fill the container width.
                    st.plotly_chart(
                        fig_transit,
                        use_container_width=True,
                    ...
                    with st.spinner("Running LOOM pipeline (gtfs2graph → topo → loom → transitmap)..."):
                        svg_text, loom_error = generate_loom_svg(
                    ...
                    if loom_error:
                        st.error(
                            "LOOM pipeline failed. Make sure the Docker image is built "
                            "(`docker build -t loom .` from the cloned ad-freiburg/loom repo) "
                            "and Docker Desktop is running."
                        )
                        st.code(loom_error, language="text")
                    else:
                        html_out = wrap_svg_centered(
                            svg_text,
                            title_text="Transit Map of Kathmandu",
